"""
Módulo de machine learning para modelo LightGBM

Este arquivo implementa um modelo LightGBM para previsão
...
class LightGBMModel:
    """
    Classe para criação, treinamento e previsão usando modelo LightGBM
    para séries temporais financeiras.
    """
    
    def __init__(self, num_leaves=31, learning_rate=0.05, n_estimators=100, sequence_length=10, model_path=None):
        """
        Inicializa o modelo LightGBM.
        
        Args:
            num_leaves (int): Número máximo de folhas em cada árvore
    ...
    def _create_features(self, data):
        """
        Cria features para o modelo LightGBM.
        ...
        for i in range(len(df) - self.sequence_length):
            ...
            feature_cols = [
                ...
                'Volume_Change', 'Volume_SMA_5', 'Volume_SMA_10',
    ...
    def train(self, data, test_size=0.2, verbose=1):
        """
        Treina o modelo LightGBM com os dados fornecidos.
        ...
        # Cria conjuntos de dados para LightGBM
        ...
        # Treina o modelo
        if verbose:
            print(f"Treinando modelo LightGBM com {self.n_estimators} árvores...")
    ...
    def predict(self, data, days_ahead=30):
        ...
        for i in range(days_ahead):
            ...
            # Calcula intervalo de confiança (simulado)
            # Em um caso real, poderia usar bootstrapping ou outros métodos
            volatility = current_data['Close'].pct_change().std() * 100  # Volatilidade histórica em %
            confidence_interval = predicted_price * (volatility / 100) * 1.96  # Intervalo de 95%
    ...
    def save(self, model_path=None):
        ...
        if save_path is None:
            ...
            save_path = f"lgb_model_{timestamp}"
        
        # Cria o diretório se não existir
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        
        # Salva o modelo
        self.model.save_model(f"{save_path}.txt")
    ...
    def plot_feature_importance(self, data):
        ...
        """
        if self.model is None:
            raise ValueError("O modelo precisa ser treinado antes de plotar a importância das features.")
        ...
        for i in range(self.sequence_length):
            ...
            feature_cols.extend([
                f'Close (t-{day})', f'SMA_5 (t-{day})', f'SMA_10 (t-{day})', f'SMA_20 (t-{day})', f'SMA_50 (t-{day})', 
                f'Return_1d (t-{day})', f'Return_5d (t-{day})', f'Return_10d (t-{day})', f'Return_20d (t-{day})',
                f'Volatility_5d (t-{day})', f'Volatility_10d (t-{day})', f'Volatility_20d (t-{day})',
                f'Volume_Change (t-{day})', f'Volume_SMA_5 (t-{day})', f'Volume_SMA_10 (t-{day})',
                f'Price_SMA5_Ratio (t-{day})', f'Price_SMA20_Ratio (t-{day})', f'SMA5_SMA20_Ratio (t-{day})',
                f'ROC_5 (t-{day})', f'ROC_10 (t-{day})', f'ROC_20 (t-{day})',
                f'EMA_5 (t-{day})', f'EMA_10 (t-{day})', f'EMA_20 (t-{day})',
                f'RSI (t-{day})'
            ])
        
        # Limita ao número real de features
        feature_cols = feature_cols[:self.model.num_feature()]
    ...
    def plot_predictions(self, data, predictions):
        ...
        # Plota as previsões
        ax.plot(dates, predicted_prices, label='Previsões', color='purple', linestyle='--')
        
        # Plota o intervalo de confiança
        ax.fill_between(dates, lower_bounds, upper_bounds, color='purple', alpha=0.2, label='Intervalo de Confiança')
        
        # Configurações do gráfico
        ax.set_title(f"Previsão de Preços para {predictions['ticker']} (LightGBM)")
        ax.set_xlabel('Data')
        ax.set_ylabel('Preço')
        ax.legend()
        ax.grid(True)
